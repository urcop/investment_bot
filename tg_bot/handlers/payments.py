...
from tg_bot.keyboards.inline.payment import payment_method_keyboard
from tg_bot.keyboards.reply.main_menu import main_menu, back_main_menu
...
async def get_payment(message: types.Message):
    await message.answer('💳 Введите сумму в рублях', reply_markup=back_main_menu)
        ...
        try:
            ...
            if data['amount'] >= 1:
                await message.answer(f'Вы хотите пополнить баланс на {data["amount"]} руб.\n'
                                     f'Выберите способ оплаты:', reply_markup=payment_method_keyboard)
            else:
                await message.answer('Минимальное пополнение от 1 рубля')
        except ValueError:
...
async def payment_success(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await User.give_balance(user_id=call.from_user.id, count=int(payment.amount), session_maker=session_maker)
        ...
        await call.message.delete()
        await call.message.answer('Успешно оплачено', reply_markup=main_menu)
    await state.finish()
